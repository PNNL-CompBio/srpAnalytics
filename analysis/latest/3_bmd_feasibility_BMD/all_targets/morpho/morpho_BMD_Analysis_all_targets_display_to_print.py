...
#complete_file_path = '/Users/kimd999/research/projects/toxicity/per_each_data/phase_I_II/input/morpho/wide/2_after_fixing_MO24/zf_morphology_data_335_chemicals_2020DEC16_fixed_wide_DNC_0_full.csv'
# 335 chemicals
...
complete_file_path = '/people/kimd999/tox/phase_I_II/morpho/wide/2_after_fixing_MO24/zf_morphology_data_335_chemicals_2020DEC16_fixed_wide_DNC_0_full.csv'
...
unique_chemical_id_s = np.unique(morpho_data['chemical.id'])
...
if ("PAH" in complete_file_path):
    ...
    morpho_data['ALL_BUT_MORT'] = morpho_data[['AXIS', 'BRN_', 'CRAN', 'DP24', 'EDEM',                                                              'LTRK', 'MUSC', 'NC__', 'SKIN', 'SM24', 'TCHR']].sum(axis=1,skipna=True,min_count=1)
else:
    # 1. Any effect at 24hrs (combination of MO24, DP24 and SM24) >> 'ANY24'
    # 2. Any effect within 5 days (combination of all measurements at both time points)
    # 3. Total mortality (MO24 + MORT) >> 'TOT_MORT'
    ...
    morpho_data['ANY120'] = morpho_data[['MORT', 'YSE_', 'AXIS', 'EYE_', 'SNOU', 'JAW_', 'OTIC',                                                        'PE__', 'BRAI', 'SOMI', 'PFIN', 'CFIN', 'PIG_', 'CIRC',                                                        'TRUN', 'SWIM', 'NC__', 'TR__', 'ANY24']].sum(axis=1,skipna=True,min_count=1)
    morpho_data['TOT_MORT'] = morpho_data[['MO24','MORT']].sum(axis=1,skipna=True,min_count=1)
    morpho_data['ALL_BUT_MORT'] = morpho_data[['DP24','SM24','NC24', 'YSE_', 'AXIS', 'EYE_', 'SNOU', 'JAW_', 'OTIC',                                                        'PE__', 'BRAI', 'SOMI', 'PFIN', 'CFIN', 'PIG_', 'CIRC','TRUN', 'SWIM', 'NC__',                                                        'TR__']].sum(axis=1,skipna=True,min_count=1)
