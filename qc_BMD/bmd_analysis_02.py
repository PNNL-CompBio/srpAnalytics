...
def runBmdPipeline(complete_file_path, full_devel):
    ...
    if test_data_sim == 0:
        ...
        morphological_data['ANY24'] = morphological_data[['MO24',\
                                                          'DP24', 'SM24']].sum(axis=1, \
                                                                               skipna=True,\
                                                                               min_count=1)

        # 2. Any effect within 5 days (combination of all measurements at both time points)
        morphological_data['ANY120'] = morphological_data[['AXIS', 'BRN_', \
                                                           'CRAN', 'EDEM', 'LTRK',\
                                                           'MORT', 'MUSC', 'NC__',\
                                                           'SKIN', 'TCHR', \
                                                           'ANY24']].sum(axis=1, \
        ...
        morphological_data['TOT_MORT'] = morphological_data[['MO24',\
                                                             'MORT']].sum(axis=1, \
                                                                          skipna=True, min_count=1)

        # 4. Any effect except mortality (#2 minus MO24 and MORT) >> 'ALL_BUT_MORT'
        morphological_data['ALL_BUT_MORT'] = morphological_data[['AXIS', 'BRN_', \
                                                                 'CRAN', 'DP24', 'EDEM', \
                                                                 'LTRK', 'MUSC', 'NC__', \
