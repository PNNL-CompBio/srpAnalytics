...
tab<- deg2 |>
  ...
  distinct()|>
  arrange(desc(nGenes))|>
  dplyr::select(PREFERRED_NAME,AVERAGE_MASS,MOLECULAR_FORMULA,Conc,nGenes,chemical_class)|>
  distinct()
...
ggplot(tab,aes(x=AVERAGE_MASS,y=nGenes,col=chemical_class))+geom_point()
...
for(genename in fintab$Gene[1:5]){
  ...
  p<-ggplot(genevals,aes(x=cas_number,y=Log2FoldChange,fill=cas_number))+geom_bar(stat='identity',position='dodge')+
    ggtitle(paste('Chemicals that induce significant changes in',genename))+
      theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
  ...
  tab<-subset(exp,Gene==genename)|>
    ...
    distinct()|>
    arrange(adj_p_value)|>
  dplyr::select(PREFERRED_NAME,Conc,Log2FoldChange,adj_p_value)|>
  distinct()
