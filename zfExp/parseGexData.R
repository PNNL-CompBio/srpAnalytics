...
generateGeneExamples <- function(genelist, chems) {
  ...
  for (gene in testgenes) {
    p1 <- subset(genelist, Gene == gene) |> ggplot(aes(x = Log2FoldChange, y = LogPValue, col = Significant, label = cas_number)) +
      geom_label_repel() +
      geom_point()
    p2 <- subset(genelist, Gene == gene) |> ggplot(aes(x = Direction, fill = Significant)) +
...
generateChemicalExamples <- function(genelist, deglist) {
  ...
  for (chem in testchems) {
    genes <- subset(genelist, Chemical_ID == chem) |>
      ...
      ggplot(aes(x = Log2FoldChange, y = Pvalue, col = Significant, alpha = 0.5, shape = concentration)) +
      geom_point()
    ...
    genecount <- subset(deglist, Chemical_ID == chem) |>
      ...
      ggplot(aes(x = Direction, fill = concentration)) +
      geom_bar(position = "dodge")
    ...
    paths <- subset(deglist, Chemical_ID == chem) |>
      ...
      ggplot(aes(x = reorder(Term, geneCount), y = geneCount, fill = -1 * log10(adj_p_value))) +
...
doEnrich <- function(genelist) {
  condition <- genelist |>
    dplyr::select(Chemical_ID, concentration) |>
    distinct()
...
main <- function(args = c()) {
  ...
  chem <- readr::read_csv(args[2]) |>
    dplyr::select(Chemical_ID, cas_number) |>
    distinct()

  ind_res <- tab %>%
    tidyr::pivot_longer(ends_with("Indication"),
  ...
  treat_names <- data.frame(
    treatment = c(
      "Acenaph_16PAH", "BbF_16PAH", "BjF_16PAH", "BkF_16PAH",
    ...
    Conc = c(
      "50uM", "50uM", "14.9uM", "1.9uM", "10uM", "5uM", "50uM", "12.2uM",
      "50uM", "50uM", "1.9uM", "16.2uM", "50uM", "50uM", "1uM", "10uM",
      "10uM", "2uM", "10uM", "10uM", "3uM", "85uM", "19.8uM", "4uM",
      "4uM", "72uM", "85uM", "85uM", "3uM", "8uM", "158uM"
    ),
    cas_number = c(
      "83-32-9", "205-99-2", "205-82-3", "207-8-9", "189-64-0", "189-55-9", "206-44-0", "65996-93-2",
